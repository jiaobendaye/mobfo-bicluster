...
%GoldsteinPrice, Himmelblau, 
Cost = @Himmelblau;
for i = 1:s     % 产生初始细菌个体的位置
    P(1,i,1,1) = -bounds + 2*rand*bounds;
    P(2,i,1,1) = -bounds + 2*rand*bounds;
end
%------------------细菌趋药性算法循环开始---------------------
%-----(2)驱散(迁移)操作开始-----
for l = 1:Ned        
    ...
    for k = 1:Nre   
        ...
        for j = 1:Nc  
            ...
            for i = 1:s
                ...
                %----------复制第j次趋化时的适应度值-----
                J(i,j,k) = Cost(P(:,i,j,k));
                %-----（4.3)修改函数，加上其它细菌对其的影响-----
                % Jcc = sum(-d_attract*exp(-ommiga_attract*(sum((P(:,i,j,k,l)-P(:,1:s,k,l)).^2)))) +...
                % sum(h_repellant*exp(-ommiga_repellant*(sum((P(:,i,j,k,l)-P(:,1:s,k,l)).^2))));
                ...
                P(:,i,j+1,k) = P(:,i,j,k) + C(i,k)*PHI;
                %-----(4.7)计算细菌当前位置的适应度值-----
                J(i,j+1,k) = Cost(P(:,i,j+1,k));
                %-----(4.8)游动-----
                m = 0; % 给游动长度计数器赋初始值
                while(m < Ns) % 未达到游动的最大长度，则循环
                    ...
                    % 存储为细菌i目前最好的适应度值
                    if(J(i,j+1,k) < Jlast)
                        Jlast = J(i,j+1,k);  %保存更好的适应度值
                        % 在该随机方向上继续游动步长单位,修改细菌位置
                        P(:,i,j+1,k) = P(:,i,j+1,k) + C(i,k)*PHI;
                        % 重新计算新位置上的适应度值
                        J(i,j+1,k) = Cost(P(:,i,j+1,k));
                    else
                        % 否则，结束此次游动
            ...
            axis([-bounds bounds -bounds bounds]); % 设置图的坐标图
            pause(.1) % 暂停0.1秒后继续
    ...
    for m = 1:s
        ...
        if(Ped > rand)
            P(1,m,1,1) = -bounds + 2*rand*bounds;
            P(2,m,1,1) = -bounds + 2*rand*bounds;
        else
            P(:,m,1,1) = P(:,m,1,Nre+1);  % 未驱散的细菌
        end
    end
